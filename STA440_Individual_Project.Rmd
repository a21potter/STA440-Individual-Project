...
In examining the results of the logistic regression model (Table 1 above), I was first interested in examining associations between the odds of a successful batted ball event and the location of the pitch, high or low. I found evidence of a significant association between the vertical pitch location and the odds of a successful batted ball, holding all other variables constant. That is, pitches that resulted in a pop fly, I found that those that were low strikes were expected to have 1.175	times the odds of being a successful hit compared to a high strike. We found that the confidence interval for this estimate does not cross 1  ([1.011, 1.365]), so there is evidence of significant association between the odds of a successful hit and vertical pitch location, for these pitches.


#####################3
However, I then proceeded to look closer into pitch location by looking at interaction effects between whether the pitch was a high or low strike and the type of batted ball. In doing so, I found evidence of a significant association between the pitch location and the batted ball being a ground ball, with all other variables held constant. That is, for pitches that are low strikes, batted balls that are ground balls are expected to have 1.893 [1.615, 2.219] times the odds of being a successful hit compared to those resulting from a high strike. That means that ground balls are associated with higher odds of a successful hit when the pitch is a low strike, but separately it does not seem to be associated with as great of a positive increase to the odds in comparison to other types of batted balls. This does not support my initial reasoning for including this interaction term, although it makes sense if we infer that it may be easier to put power behind, but not levitate, a higher pitch.
##################

Similarly, PCI is associated with higher hazard for patients with a history of diabetes. That is, for individuals with a history of diabetes, patients who received the PCI catheterization are expected to have 2.251 [1.992, 2.698] times the hazard compared to an individual who received the CABG treatment. This supported our hypothesis that PCI may be associated with worse outcome for patients who have more severe/complicated conditions.

Among the variables I controlled for, I noticed that hit distance had a significant negative association with the odds of a hit being successful. Holding all other variables in the model constant, for every additional foot of projected distance, the odds of a successful hit are expected to decrease by a factor of 0.998 ([0.998, 0.999]). I found this interesting because it suggests that hits that travel greater distances don't have the positive correlation with hit success that we might assume. 

Additionally, I made note that the type of batted ball (line drives and popups) also had a significant association with the odds of a pitch resulting in a successful hit. For high strikes that result in a line drive, the odds of a successful hit are expected to increase by a factor of 12.787	([11.176, 14.662]), holding all other variables constant. Similarly, for high strikes that result in a popup, the odds of a successful hit are expected to decrease by a factor of 0.143	([0.088, 0.219]), holding all other variables constant.

Interestingly, there was not evidence of statistically significant associations between the odds of a successful batted ball and pitcher related variables like the pitcher handednes, the release speed, or the type of pitch.
