...
Separately, this analysis will also be looked at through the lens of the following question: Although shifting the strike zone up or down would be a slowly implemented change that would have to begin with lower level leagues and younger players so as to not cause injury or upset among players, are either of these options reasonable for increasing action?
...
statcast_data_filtered <- statcast_data_2021 %>%
  ...
  filter(pitch_type != "EP", pitch_type != "KN", pitch_type != "CS") %>%
...
Because of the vague wording of the definition of the strike zone and the dependency the called strike zone has on the judgement of the umpire, I have included a plot displaying the X and Z coordinates of all high and low strikes relevant to this analysis. In this case, the x-axis refers to the horizontal distance of the pitch from the center of home plate (in ft) while the y-axis refers to the height of the pitch (in ft) as it crosses home plate. This plot helps us visualize the difference between high and low strikes as was observed in 2021. As there is no definitive cutoff for the top or bottom of the strike zone, understanding the observed heights of these strikes is important to understanding how pitch location may be related to balls in play. As is shown, the observed bottom of the strike zone appears to be slightly below 1.5 ft while the observed top of the strike zone appears to be slightly above 3.5 ft. Noticeably, both the observed top region and bottom region of the strike zone appear to be around a foot in height each, which is taller than the portion of the strike zone deemed the vertical middle.
...
statcast_data_filtered %>%
  ...
  labs(title = "Height of high strikes around 3 ft, \nheight of low strikes around 2 ft",
       x = "Horizontal loction of pitch (ft from center of the strike zone)",
       y = "Vertical loction of pitch \n(ft from the ground)",
       caption = "Visualizing the 2021 strike zone;\nlocations of high and low strikes",
...
$$
\begin{aligned}
\log(\frac{p_i}{1 - p_i}) &= \beta_0+\beta_1(\text{Pitch Zone (Low)}_i)+\beta_2(\text{Pitch Type (Changeup)}_i)
...
Where $i$ refers to each pitch resulting in a batted ball event and $p$ refers to the predicted probability of a successful batted ball.
...
The logistic model assumes that there is a linear relationship between the logit of the outcome and each continuous predictor variable. To check the linearity assumption, empirical logit plots were observed to assess the relationships between the continuous predictor variables launch speed and release speed and the log odds of the outcome variable (Appendix E). In regard to the empirical logit plot for release speed, it appears that release speed is relatively linearly associated with the log odds of whether batted balls are successful or not. However, because the empirical logit plot for launch speed shows linearity being violated considering the presence of a strong "U" shape, it is evident that launch speed does not have a linear relationship with the log odds of whether batted balls are successful or not. To avoid this linearity violation, I instead include a transformed version of launch speed (launch speed$~^5$) as it has a much more linear relationship with the log odds of whether batted balls are successful or not.
...
In examining the results of the logistic regression model (Table 1 above), I was first interested in examining associations between the odds of a successful batted ball event and the location of the pitch, high or low. I found evidence of a significant association between the vertical pitch location and the odds of a successful batted ball, holding all other variables constant. That is, for pitches that resulted in a fly ball, I found that those that were low strikes were expected to have 1.163 times the odds of being a successful hit compared to a high strike. We found that the confidence interval for this estimate does not cross 1 ([1.002, 1.350]), so there is evidence of significant association between the odds of a successful hit and vertical pitch location, for these pitches.


However, I then proceeded to look closer into pitch location by looking at interaction effects between whether the pitch was a high or low strike and the type of batted ball. In doing so, I found evidence of a significant association between the pitch location and the batted ball being a ground ball, with all other variables held constant. That is, for pitches that are low strikes, batted balls that are ground balls are expected to have 1.905 [1.627, 2.230] times the odds of being a successful hit compared to those resulting from a high strike. That means that ground balls are associated with higher odds of a successful hit when the pitch is a low strike, but separately ground balls on low strikes do not seem to be associated with as great of a positive increase to the odds in comparison to other types of batted balls on low strikes. This is interesting because although the increase in the odds is expected according to my original justification of including this interaction effect, its comparison to the interaction effects of other types of batted balls on low strikes is not something I had expected.

Among the variables I controlled for, I noticed that for high strikes, the type of batted ball has significant associations with the odds of a successful hit. Holding all other variables in the model constant, for high strikes resulting in line drives the odds of a successful hit are expected to increase by a factor of 14.353 ([12.603, 16.381]) compared to fly balls. This made sense as line drives are typically hard hit balls but lack the elevation that potentially allows the defense higher chances of making a catch. Holding all other variables in the model constant, for high strikes resulting in ground balls the odds of a successful hit are expected to increase by a factor of 4.146 ([3.643, 4.727]) compared to fly balls. I was slightly surprised by this relationship in comparison to fly balls because it suggests ground balls are more likely to result in a successful hit than a fly ball on high strikes. As fly balls are often the byproduct of failed home run attempts, this association supports the idea that aiming to put balls in play rather than prioritizing home runs may increase game action due to more successful balls in play. Holding all other variables in the model constant, for high strikes resulting in popups the odds of a successful hit are expected to decrease by a factor of 0.192 ([0.119, 0.292]) compared to fly balls. This is not surprising at all as popups are simply fly balls that don't reach the outfield, and with a higher density of infielders than outfielders in their respective areas, they are more likely to be caught.
...
Another limitation of this model is its reliance on defensive action and skill. Because I chose to only consider balls hit in play, the deciding factor of whether the hit is successful or not depends on if the defense was successful in playing the ball and getting the batter out. Without quantifying or controlling for the defense's contribution to the play outcome, this analysis may be missing additional patterns in the data.
...
Separately, it would be interesting to consider the further implications of changing the strike zone such as how pitcher's pitch choice would change to best take advantage of a new strike zone.
...
Three True Outcomes: Glossary. MLB.com. (n.d.). Retrieved December 16, 2022, from https://www.mlb.com/glossary/idioms/three-true-outcomes
...
- AB Automatic Ball, AS Automatic Strike, CH Change-up, CU Curveball, EP Eephus, FA Fastball, FC Cutter, FF Four-Seam Fastball, FO Forkball, FS Splitter, FT Two-Seam Fastball (synonymous with SI), GY Gyroball, IN Intentional Ball, KC Knuckle Curve, KN Knuckleball, NP No Pitch, PO Pitchout, SC Screwball, SI Sinker (synonymous with FT), SL Slider
