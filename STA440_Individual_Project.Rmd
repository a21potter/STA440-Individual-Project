...
```{r}
statcast_data_filtered %>%
  group_by(zone_high_low, events, babip_value) %>%
  summarize(count_babip = n()) %>%
  summarize(zone_high_low, events, babip_value, count_babip, prop_babip = count_babip/sum(count_babip)) %>%
  ggplot(aes(x = events, y = prop_babip, fill = babip_value)) +
  geom_bar(stat = "identity") +
  facet_wrap(~zone_high_low)
```
